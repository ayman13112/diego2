...
const IndRegisteredCourse2 = () => {
  ...
  const handleReqRefund = () => {
    axios
     .post(`/api/individualTrainee/reqrefund/${id1}/${id2}`)
     .then((res) => {
        console.log(res);
  ...
  return (
    <article>
      <div>
        ...
        <button onClick={handleReqRefund}> Refund your money</button>
        <br />
        <button onClick={()=>window.location.href=`/IndWriteNotes/${id1}/${id2}`}>Write Notes</button>
      </div>
    </article>
  );
};

export default IndRegisteredCourse2;
