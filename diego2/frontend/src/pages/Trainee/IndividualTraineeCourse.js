...
const TraineeCourse = () => {
  ...
  const handlePayCourse = () => {
    axios.post(`/api/individualTrainee/payonline/${id1}/${id2}`)
  ...
  return (
    <article>
      <div>
       ...
       <button onClick={()=>window.location.href=`/IndReportProblem/${id1}/${id2}`}>Report a problem</button>
       <br />
       <button onClick={handlePayCourse}>Pay for Course</button>
      </div>
    </article>
  );
};
