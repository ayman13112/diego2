...
const CorpViewAllProblem = () => {
  ...
  const getCourses = async () => {
    ...
    await axios.get(`/api/corporateTrainee/seelist/${id}`).then(
      (res) => {
        // console.log(res);
      ...
      <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
        ...
        <TableBody>
          {courses.map((course) => (
            ...
            onClick={() => window.location.href=`/CorpReportFollowUp/${id}/${course._id}`}
            //   key={blog._id}
...
export default CorpViewAllProblem